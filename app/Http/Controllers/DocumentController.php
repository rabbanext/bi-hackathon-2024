...
class DocumentController extends Controller
{
    public function submitDocument(Request $request)
    {
        ...
        if (!$request->hasFile('document_file')) {
            return redirect()->back()->withErrors('Please provide either a document for validate. e.g: KTP, KTM.');
        }
        // if total document is not same with length of member name, return error
        $members = Auth::user()->member_name ? explode(',', Auth::user()->member_name) : [];
        if ($request->hasFile('document_file') && count($members) != count($request->file('document_file'))) {
